...
다른 운영체제의 관리하에 어떤 운영체제를 가상 운영체제로 실행할 수 있다. VMWare, VirtualBox, Xen
...
운영체제에서 물리저장 매체를 파일과 폴더의 계층 구조처럼 보이게 하는 부분

논리적 구성과 물리적 구현 간의 분리를 보여 준다.

반면에 운영체제는 모두 동일한 인터페이스로 표시한다

디렉터리(폴더) 조직화된 구조를 제공

**파일**

문서, 사진, 음악, 스프레드시트, 웹페이지 등의 실질적인 내용을 담는다.



파일이 어디 있는지, 이러한 정보를 관리하면서 애플리케이션이나 운영체제의 나머지 부분이 정보를 읽고 쓸 수 있도록 접근 가능하게 만든다.파일에 대한 접근이 효율적으로 수행되고 서로 간섭되지 않도록 조정하는 역할을 하고, 데이터의 물리적인 위치를 계속 파악한다.

가장 낮은 레벨의 시스템 콜을 통해 사용할 수 있으며, 파일 서비스를 마늗ㄹ 때 공통적인 부분은 라이브러리에 제공되어있어 프로그래밍하기 쉽다.



#### 보조 기억 장치 파일 시스템

파일 시스템이 한 파일이 사용하는 바이트를 다른 파일이 사용하는 바이트와 같은 블록에 저장하지 않는다. 그러므로 빈 공간을 낭비하는 경우가 생긴다.

폴더 엔트리

파일이 드라이브 어디에 저장되어 있는지, 블록 중 어느 것이 파일의 바이트를 담고 있는지에 대한 정보를 넣고 있다.

SSD

하드 드라이버와 기본 아이이어는 같다. 그러나 다른 드라이버를 사용하고, 장치 자체에 정보가 어느 위치에 있는지 기억하기 위한 정교한 코드가 들어있다(SSD 장치의 각 영역이 사용 될 수 있는 횟수에 제한이 있기 때문이다.)

웨어 레벨링: 각 물리적 블록이 몇 번 사용되었는지 파악하고, 각 블록이 거의 같은 횟수로 사용되도록 데이터를 옮긴다.
...
모든 브라우저에 포함되어 있어 소프트웨어를 별도로 다운하지 않아도 된다.

언어 자체가 간단하고, 경험을 조금 쌓아도 좋은 코드를 작성할 수 있다.

언어가 굉장히 유연하다

웹 사이트에 다양한 효과를 줄 수 있으며, 웹 사이트에서 제공하는 API를 제공하기 위한 언어다.
...
브라우저 인터페이스는 생각보다 표준화되어 있지 않아 프로그램이 서로 다른 브라우저에서 항상 같은 방식으로 작동하지는 않는다.



자바스크립트를 쓰지 않는 이유? : https://youtu.be/8YIwaO6Cojw

1+True = 2 와 같은 코드를 실행시킨다🙃
...
프로그래밍 입문이나 데이터 과학, 머신러닝에 파이썬이 자주 이용된다.

웹에서도 실행 가능한 사이트가 있어 별도의 소프트웨어를 다운로드하거나 명령줄 프로그래밍 인퍼테이스를 익히지 않아도 된다.



### 프로그래밍 언어의 주요 개념

프로그래밍 언어는 계산을 일련의 단계로 풀어서 상세히 설명한다

> 입력 데이터를 읽음👉 산술연산 👉 중간값 저장/계산 
>
> 👉 이전 값을 기반으로 진행 결정 & 결과 표시 👉 결과 저장



#### 구문 규칙(Syntax)

문법적으로 무엇이 맞고 무엇이 틀리는지 정의 (상당히 까다로움)
...
- 컴퓨터에 연산을 수행하고, 조건을 검사하고, 계산을 반복하라고 지시하는 문장



2. **코드 라이브러리**

- 다른 이들이 작성했지만 프로그램을 만들 때 사용할 수 있는 것 (사전 제작된 부품)
- 예) 수학 연산, 날짜 계산, 텍스트 검색과 조작을 위한 함수



3. **프로그램 실행 환경에 접근**

- 자바스크립트 프로그램은 <u>브라우저</u>에서 제어/실행될 경우 사용자에게서 입력을 받고 버튼을 누르는 등의 이벤트에 반응한다
- 파이썬은 <u>실행되는 컴퓨터</u>상의 파일 시스템에 접근할 수 있다
